...
function build(syscall, _state, makeRoot, forVatID) {
  ...
  function queueMessage(targetSlot, prop, args) {
    ...
    lsdebug(`ls[${forVatID}].qm send(${inspect(targetSlot)}, ${prop}, ${inspect(ser.argsString)} ${inspect(ser.slots)} -> ${promiseID}`);
  ...
  function notifyFulfillToData(promiseID, data, slots) {
    lsdebug(`ls[${forVatID}].dispatch.notifyFulfillToData(${promiseID}, ${data}, ${inspect(slots)})`);
  ...
  function notifyFulfillToPresence(promiseID, slot) {
    lsdebug(`ls[${forVatID}].dispatch.notifyFulfillToPresence(${promiseID}, ${inspect(slot)})`);
    if (!importedPromisesByPromiseID.has(promiseID)) {
      throw new Error(`unknown promiseID '${promiseID}'`);
    }
